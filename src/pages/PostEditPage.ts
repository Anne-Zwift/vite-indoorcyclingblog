...
export function PostEditPage(id?: string): HTMLDivElement {
  ...
  const handleUpdate = async (postData: PostRequest) => {
    try {
      ...
      showTempMessage(
        document.body,
        'Failed to save changes. Please check the console.',
        true,
      );
      throw error;
    }
  ...
  getPostDetails(id)
    .then((post) => {
      ...
      pageContainer.append(editForm);
    })
    .catch((error) => {
      console.error('Failed to fetch post details:', error);
      pageContainer.innerHTML =
        '<h2>Error: Could not load post data.</h2><p>Please check the console for details.</p>';
    });
  return pageContainer;
}
